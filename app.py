...
def index():
    ...
    stats = {
        'most_categories' : top_category.reset_index()['Category'][0],
        'total' : top_category['Jumlah'][0],
        'rev_table' : playstore.sort_values('Reviews', ascending=False).reset_index()[['Category','App','Reviews','Rating']].head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    playstore['Installs'].head(40).plot()
    figfile = BytesIO()
    plt.savefig(figfile, format='png')
    figfile.seek(0)
    figdata_png = base64.b64encode(figfile.getvalue())
